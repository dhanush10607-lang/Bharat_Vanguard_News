...
Since Bharat Vanguard News (BVN) already uses Supabase as its database, we use **Supabase Auth** to handle the Google OAuth flow. This means:
...
6. Set the name to `Bharat Vanguard News`
  ...
  useEffect(() => {
    ...
    if (accessToken) {
      ...
      fetch('/api/v1/auth/google/verify', {
        ...
        .then(data => {
          // Store our app JWT
          localStorage.setItem('bvn_token', data.access_token);
          localStorage.setItem('bvn_user', JSON.stringify(data.user));
