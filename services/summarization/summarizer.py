"""
Bharat Vanguard News (BVN) — AI Summarizer
    ...
    def summarize_article(self, text: str) -> dict:
        ...
        try:
            ...
            if "summary_text" in result[0]:
                ...
                medium_text = result[0]["generated_text"].strip()
                # Text generation sometimes returns the input prepended, so strip it out if needed
                if medium_text.startswith(safe_text):
                    medium_text = medium_text[len(safe_text):].strip()
            else:
                medium_text = str(result[0]).strip()
